...
void main(void)
{
	...
	gl_Position = u_mvp * a_instance * skinRes * vec4(a_position, 1.0);

	v_tex = a_tex;
    v_color = a_color;
	v_normal = TransformNormal(a_normal, u_model * a_instance);
	v_world = vec3(u_model * a_instance * vec4(a_position, 1.0));
}
